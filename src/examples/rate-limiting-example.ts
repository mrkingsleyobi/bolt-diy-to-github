...
async function basicRateLimitingExample() {
  ...
  const rateLimiter = new RateLimitingService(5, 1, 'example-secret-key');
...
async function authenticatedCommunicationExample() {
  ...
  // Create services
  const rateLimiter = new RateLimitingService(10, 2, 'communication-secret');
  ...
  // Encrypt response
  const encryptedResponse = await rateLimiter.encryptWithRateLimit(
  ...
  const decryptedResponse = await rateLimiter.decryptWithRateLimit(
    encryptedResponse,
    'response-encryption-key'
  );
...
function configurationManagementExample() {
  ...
  const rateLimiter = new RateLimitingService(10, 1, 'config-secret');
...
async function errorHandlingExample() {
  ...
  const rateLimiter = new RateLimitingService(5, 1, 'error-secret');
...
async function chromeExtensionIntegrationExample() {
  ...
  const handleExportRequest = async (projectId: string) => {
