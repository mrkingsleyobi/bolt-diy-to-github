...
import os from 'os';
import { createTestZip } from './zip-example-helpers.js';
...
async function runExample() {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'zip-example-'));
  const zipFilePath = path.join(tempDir, 'example.zip');
  ...
  try {
    ...
    const result1 = await ZipExtractionService.extract(zipFilePath, extractDir);
    ...
    const result2 = await ZipExtractionService.extract(zipFilePath, extractDir + '_limited', {
    ...
    const result3 = await ZipExtractionService.extract(zipFilePath, extractDir + '_progress', {
      overwrite: true,
      onProgress: (progress) => {
