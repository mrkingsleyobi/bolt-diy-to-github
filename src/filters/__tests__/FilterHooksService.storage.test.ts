...
(global as any).mcp__claude_flow__memory_usage = jest.fn();

describe('FilterHooksService Persistent Memory Storage', () => {
  let hooksService: FilterHooksService;
  let mockMcpMemoryUsage: jest.Mock;

  beforeEach(() => {
    hooksService = FilterHooksService.getInstance();
    mockMcpMemoryUsage = (global as any).mcp__claude_flow__memory_usage as jest.Mock;
  ...
  it('should handle truth scores above 0.95 threshold', async () => {
    ...
    const truthScoreCall = calls.find(call =>
      call[0].key && call[0].key.includes('truth-score')
    );
