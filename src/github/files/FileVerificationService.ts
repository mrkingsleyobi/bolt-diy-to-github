...
export class FileVerificationService {
  private static instance: FileVerificationService;
  ...
  private calculateOperationAccuracy(
    ...
    if (operations.length === 0) return 1; // Perfect score for no operations

    // Check if all operations have corresponding results
    const matchingResults = results.filter(result =>
      operations.some(op => op.path === result.path)
    );
  ...
  private calculateResultConsistency(
    ...
    if (totalOperations === 0) return 1; // Perfect consistency for no operations
  ...
  private calculateErrorHandlingQuality(
    ...
    if (failedResults.length === 0) return 1; // Perfect score when no failures

    // Count how many failures have meaningful error messages
    const meaningfulErrors = failedResults.filter(result =>
      result.error && result.error.length > 10 // At least 10 characters for a meaningful error
    ).length;
