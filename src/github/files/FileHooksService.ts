 ...
 */
export class FileHooksService {
  private static instance: FileHooksService;
   ...
   */
  async postEdit(filePath: string, operation: string, result: any): Promise<void> {
    try {
      const editData = {
        sessionId: this.sessionId,
        file: filePath,
   ...
   */
  async postTask(operationSummary: FileOperationSummary, truthScore?: number): Promise<void> {
    try {
      const taskData = {
        sessionId: this.sessionId,
