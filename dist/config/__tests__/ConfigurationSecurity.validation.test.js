...
const GitHubAppAuthService_1 = require("../../services/GitHubAppAuthService");
...
describe('Configuration Management - Security Validation', () => {
    ...
    let messageAuthService;
    let githubPatAuthService;
    ...
    beforeEach(() => {
        ...
        tokenEncryptionService = new TokenEncryptionService_1.TokenEncryptionService();
        messageAuthService = new MessageAuthenticationService_1.MessageAuthenticationService();
        messageAuthService.setSecretKey('test-auth-secret-key');
        githubPatAuthService = new GitHubPATAuthService_1.GitHubPATAuthService();
        githubAppAuthService = new GitHubAppAuthService_1.GitHubAppAuthService('test-client-id', 'test-client-secret');
        basicManager = new BasicConfigurationManager_1.BasicConfigurationManager(encryptionService, messageAuthService);
        environmentService = new EnvironmentConfigurationService_1.EnvironmentConfigurationService(encryptionService, messageAuthService, tokenEncryptionService, testEncryptionPassword, githubPatAuthService, githubAppAuthService);
    });
    afterEach(() => {
    ...
    describe('Sensitive Data Protection', () => {
        it('should never expose sensitive tokens in plain text', async () => {
            ...
            const sanitized = environmentService.sanitizeForTransmission(sensitiveConfig);
    ...
    describe('Authentication and Authorization', () => {
        ...
        it('should reject messages with invalid signatures', async () => {
            ...
            const authService1 = new MessageAuthenticationService_1.MessageAuthenticationService();
            authService1.setSecretKey(secret1);
            const authService2 = new MessageAuthenticationService_1.MessageAuthenticationService();
    ...
    describe('Secure Configuration Transmission', () => {
        it('should sanitize configuration for transmission', async () => {
            ...
            const sanitized = environmentService.sanitizeForTransmission(fullConfig);
            // Verify sensitive fields are removed
            expect(sanitized.github).not.toHaveProperty('token');
        ...
        it('should maintain data integrity during sanitization', async () => {
            ...
            const sanitized = environmentService.sanitizeForTransmission(originalConfig);
    ...
    describe('Secure Error Handling', () => {
        it('should not expose sensitive information in error messages', async () => {
            ...
            try {
                await environmentService.encryptSensitiveData({
                    github: { token: null }
                });
            }
            catch (error) {
        ...
        it('should handle invalid inputs gracefully', async () => {
            ...
            for (const input of invalidInputs) {
                ...
                await expect(environmentService.validateTokens(input))
                    .rejects.toThrow(); // Should throw controlled errors, not crash
                await expect(environmentService.refreshTokens(input))
                    .rejects.toThrow(); // Should throw controlled errors, not crash
            }
        });
    });
...
//# sourceMappingURL=ConfigurationSecurity.validation.test.js.map