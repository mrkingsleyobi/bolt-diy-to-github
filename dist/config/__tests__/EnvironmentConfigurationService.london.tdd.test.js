...
const EnvironmentConfigurationService_1 = require("../EnvironmentConfigurationService");
...
describe('EnvironmentConfigurationService - London School TDD', () => {
    ...
    let mockTokenEncryptionService;
    let mockTokenValidationService;
    let mockGitHubPATAuthService;
    let mockGitHubAppAuthService;
    let mockMessageAuthenticationService;
    let mockPayloadEncryptionService;
    const testEncryptionPassword = 'test-password-123';
    beforeEach(() => {
        // Reset all mocks
        jest.clearAllMocks();
        // Create mock instances
        mockPayloadEncryptionService = new PayloadEncryptionService_1.PayloadEncryptionService();
        ...
        BasicConfigurationManager_1.BasicConfigurationManager.mockImplementation(() => mockConfigurationManager);
        // Setup TokenValidationService mock
        mockTokenValidationService = {
            validateTokens: jest.fn(),
            refreshTokens: jest.fn()
        };
        TokenValidationService_1.TokenValidationService.mockImplementation(() => mockTokenValidationService);
        // Create service instance
        service = new EnvironmentConfigurationService_1.EnvironmentConfigurationService(mockTokenEncryptionService, mockMessageAuthenticationService, mockTokenEncryptionService, // Using same mock for token encryption
        testEncryptionPassword, mockGitHubPATAuthService, mockGitHubAppAuthService);
    });
    afterEach(() => {
    ...
    describe('Constructor', () => {
        it('should create an instance with all dependencies', () => {
            expect(service).toBeInstanceOf(EnvironmentConfigurationService_1.EnvironmentConfigurationService);
            expect(BasicConfigurationManager_1.BasicConfigurationManager).toHaveBeenCalledWith(mockTokenEncryptionService, mockMessageAuthenticationService);
        });
        it('should initialize token validation service correctly', () => {
            expect(TokenValidationService_1.TokenValidationService).toHaveBeenCalledWith(mockGitHubPATAuthService, mockTokenEncryptionService, testEncryptionPassword, mockGitHubAppAuthService);
        });
    });
    ...
    describe('getEnvironmentConfig', () => {
        ...
        beforeEach(() => {
            ...
            jest.spyOn(service, 'getFullConfig').mockReturnValue(mockFullConfig);
            jest.spyOn(service, 'validateAccessTokens').mockResolvedValue(true);
            jest.spyOn(service, 'sanitizeForTransmission').mockReturnValue(mockSanitizedConfig);
        });
        ...
        it('should validate access tokens before returning config', async () => {
            mockConfigurationManager.initialize.mockResolvedValue(undefined);
            const validateSpy = jest.spyOn(service, 'validateAccessTokens').mockResolvedValue(true);
            await service.getEnvironmentConfig(testEnvironment);
            expect(validateSpy).toHaveBeenCalledWith(mockFullConfig);
        });
        it('should throw error when token validation fails', async () => {
            mockConfigurationManager.initialize.mockResolvedValue(undefined);
            jest.spyOn(service, 'validateAccessTokens').mockResolvedValue(false);
        ...
        it('should sanitize configuration before returning', async () => {
            mockConfigurationManager.initialize.mockResolvedValue(undefined);
            const sanitizeSpy = jest.spyOn(service, 'sanitizeForTransmission').mockReturnValue(mockSanitizedConfig);
            await service.getEnvironmentConfig(testEnvironment);
    ...
    describe('saveEnvironmentConfig', () => {
        ...
        beforeEach(() => {
            jest.spyOn(service, 'encryptSensitiveData').mockResolvedValue(mockEncryptedConfig);
        });
        it('should initialize configuration manager with environment-specific options', async () => {
            mockConfigurationManager.initialize.mockResolvedValue(undefined);
        ...
        it('should encrypt sensitive data before saving', async () => {
            mockConfigurationManager.initialize.mockResolvedValue(undefined);
            const encryptSpy = jest.spyOn(service, 'encryptSensitiveData').mockResolvedValue(mockEncryptedConfig);
            await service.saveEnvironmentConfig(testEnvironment, mockConfig);
        ...
        it('should handle encryption errors gracefully', async () => {
            mockConfigurationManager.initialize.mockResolvedValue(undefined);
            jest.spyOn(service, 'encryptSensitiveData').mockRejectedValue(new Error('Encryption failed'));
    ...
    describe('getFullConfig (private method)', () => {
        it('should construct full configuration from configuration manager values', () => {
            ...
            mockConfigurationManager.get.mockImplementation((key, defaultValue) => {
    ...
    describe('validateAccessTokens (private method)', () => {
        it('should return true when no tokens are present', async () => {
            const config = {};
            const result = await service.validateAccessTokens(config);
    ...
    describe('sanitizeForTransmission (private method)', () => {
        it('should remove sensitive data from configuration', () => {
            ...
            const sanitized = service.sanitizeForTransmission(config);
    ...
    describe('encryptSensitiveData (private method)', () => {
        it('should encrypt GitHub token when present', async () => {
            ...
            const encryptedConfig = await service.encryptSensitiveData(config);
        ...
        it('should handle encryption errors gracefully', async () => {
            ...
            await expect(service.encryptSensitiveData(config))
    ...
    describe('decryptSensitiveData (private method)', () => {
        it('should decrypt GitHub token when present', async () => {
            ...
            const decryptedConfig = await service.decryptSensitiveData(config);
        ...
        it('should handle decryption errors gracefully', async () => {
            ...
            await expect(service.decryptSensitiveData(config))
                .rejects.toThrow(`Failed to decrypt GitHub token: ${errorMessage}`);
        });
    });
});
//# sourceMappingURL=EnvironmentConfigurationService.london.tdd.test.js.map