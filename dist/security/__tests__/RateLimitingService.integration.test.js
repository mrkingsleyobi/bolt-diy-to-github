...
describe('RateLimitingService - Integration Tests', () => {
    ...
    beforeEach(() => {
        rateLimitingService = new RateLimitingService_1.RateLimitingService(10, 2, 'integration-test-secret');
        authService = new MessageAuthenticationService_1.MessageAuthenticationService();
        encryptionService = new PayloadEncryptionService_1.PayloadEncryptionService();
    ...
    it('should handle complete cross-origin communication flow', async () => {
        ...
        const encryptedResponse = await rateLimitingService.encryptWithRateLimit(JSON.stringify(responseData), 'response-encryption-key');
        // Step 6: Client decrypts response
        const decryptedResponse = await rateLimitingService.decryptWithRateLimit(encryptedResponse, 'response-encryption-key');
    ...
    it('should reject tokens from attackers', () => {
        ...
        const attackerService = new MessageAuthenticationService_1.MessageAuthenticationService();
    ...
    it('should work with multiple service instances', async () => {
        const service1 = new RateLimitingService_1.RateLimitingService(5, 1, 'service1-secret');
        const service2 = new RateLimitingService_1.RateLimitingService(15, 3, 'service2-secret');
    ...
    it('should handle encrypted cross-origin communication', async () => {
        ...
        const encryptedRequest = await encryptionService.encryptPayload(JSON.stringify(requestData), 'request-encryption-key');
        // Apply rate limiting
        const canProceed = rateLimitingService.consume(1);
        expect(canProceed).toBe(true);
        ...
        const decryptedRequest = await encryptionService.decryptPayload(encryptedRequest, 'request-encryption-key');
        ...
        const encryptedResponse = await encryptionService.encryptPayload(JSON.stringify(response), 'response-encryption-key');
        // Client decrypts response
        const decryptedResponse = await encryptionService.decryptPayload(encryptedResponse, 'response-encryption-key');
        const parsedResponse = JSON.parse(decryptedResponse);
        expect(parsedResponse.success).toBe(true);
    ...
    it('should synchronize rate limit configuration across services', async () => {
        // Create two services
        const serviceA = new RateLimitingService_1.RateLimitingService(10, 1, 'sync-secret');
        const serviceB = new RateLimitingService_1.RateLimitingService(5, 2, 'sync-secret');
    ...
    it('should handle errors gracefully in cross-origin communication', async () => {
        ...
        const testRateLimitingService = new RateLimitingService_1.RateLimitingService(10, 2, 'integration-test-secret');
        // Create a real PayloadEncryptionService instance for spying
        const payloadEncryptionService = new PayloadEncryptionService_1.PayloadEncryptionService();
        ...
        await expect(testRateLimitingService.encryptWithRateLimit('test data', 'key')).rejects.toThrow('Network error');
        ...
        await expect(testRateLimitingService.decryptWithRateLimit('{"encryptedMessage":{}}', 'key')).rejects.toThrow('Failed to decrypt with rate limit');
        // Restore original constructor
        jest.spyOn(require('../PayloadEncryptionService'), 'PayloadEncryptionService')
            .mockImplementation(originalConstructor);
    });
});
//# sourceMappingURL=RateLimitingService.integration.test.js.map