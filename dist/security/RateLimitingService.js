...
class RateLimitingService {
     ...
     */
    constructor(bucketSize = 10, refillRate = 1, secretKey = '') {
     ...
     */
    updateConfiguration(bucketSize, refillRate) {
     ...
     */
    createRateLimitToken(payload, expirationMinutes = 5) {
        if (!this.secretKey) {
     ...
     */
    async encryptWithRateLimit(data, secret) {
        try {
     ...
     */
    async decryptWithRateLimit(encryptedData, secret) {
        try {
            const rateLimitData = JSON.parse(encryptedData);
            const payloadEncryptionService = new PayloadEncryptionService_1.PayloadEncryptionService();
            const decryptedData = await payloadEncryptionService.decryptPayload(rateLimitData.encryptedMessage, secret);
