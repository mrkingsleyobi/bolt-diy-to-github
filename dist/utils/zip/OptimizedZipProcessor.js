...
exports.OptimizedZipProcessor = void 0;
const stream_1 = require("stream");
const yauzl_1 = __importDefault(require("yauzl"));
const zip_1 = require("../../types/zip");
...
const EntryFilter_1 = require("./EntryFilter");
...
class OptimizedZipProcessor {
    ...
    constructor(memoryLimit, maxEntriesInMemory = 1000) {
        ...
        this.smallBufferPool = [];
        this.mediumBufferPool = [];
        this.largeBufferPool = [];
        this.SMALL_BUFFER_SIZE = 4 * 1024; // 4KB
        this.MEDIUM_BUFFER_SIZE = 64 * 1024; // 64KB
        this.LARGE_BUFFER_SIZE = 1024 * 1024; // 1MB
        this.MAX_CATEGORY_POOL_SIZE = 30;
        ...
        this.entryFilter = new EntryFilter_1.EntryFilter();
    }
    /**
     ...
     */
    async extract(zipFilePath, destinationPath, options = {}) {
        ...
        try {
            ...
            if (!zipFilePath || typeof zipFilePath !== 'string') {
                throw new zip_1.ZipExtractionError('Invalid ZIP file path provided', 'INVALID_ZIP_FILE');
            }
            if (!destinationPath || typeof destinationPath !== 'string') {
                throw new zip_1.ZipExtractionError('Invalid destination path provided', 'INVALID_ZIP_FILE');
            }
            ...
            const finalResult = await new Promise((resolve, reject) => {
                yauzl_1.default.open(zipFilePath, { lazyEntries: true }, (err, zipFile) => {
                    if (err) {
                        reject(new zip_1.ZipExtractionError(`Failed to open ZIP file: ${err.message}`, 'INVALID_ZIP_FILE', err));
                        return;
                    }
                    if (!zipFile) {
                        reject(new zip_1.ZipExtractionError('Failed to open ZIP file: zipFile is null', 'INVALID_ZIP_FILE'));
                        return;
                    }
            ...
            const truthScore = this.verificationService.calculateTruthScore(mockStreamEntries, finalResult, 0);
     ...
     * @returns Extraction result with statistics
     */
    async extractStreaming(zipFilePath, destinationPath, options = {}) {
        ...
        try {
            ...
            for (const entry of filteredEntries) {
                if (entry.isDirectory) {
                    ...
                    await this.processStreamEntryOptimized(entry, destinationPath, options, result);
                }
            }
            // Record operation in agentic jujutsu
            await this.jujutsuService.recordOperation({
            ...
            // Calculate truth score using ZipVerificationService
            const truthScore = this.verificationService.calculateTruthScore(streamEntries, result, 0);
            // Call post-task hook with calculated truth score
            await this.hooksService.postTask({
            ...
            throw new zip_1.ZipExtractionError(`Streaming extraction failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'EXTRACTION_FAILED', error instanceof Error ? error : undefined);
        }
        finally {
            try {
                // Cleanup agentic jujutsu session
     ...
     */
    async extractStreamsOptimized(buffer, options = {}) {
        // Pre-task hook
     ...
     */
    async processStreamEntryOptimized(entry, destinationPath, options, result) {
        const fullPath = path_1.default.join(destinationPath, entry.name);
        ...
        try {
            ...
            return new Promise((resolve, reject) => {
                ...
                entry.stream.on('data', (chunk) => {
                    ...
                    if (this.memoryMonitor.isLimitExceeded()) {
                        ...
                        writeStream.destroy();
                        reject(new zip_1.ZipExtractionError(`Memory limit exceeded during processing of ${entry.name}`, 'MEMORY_LIMIT_EXCEEDED'));
                        return;
                    }
            ...
            throw new zip_1.ZipExtractionError(`Failed to process entry ${entry.name}: ${error instanceof Error ? error.message : 'Unknown error'}`, 'EXTRACTION_FAILED', error instanceof Error ? error : undefined);
        }
    }
    /**
     ...
     */
    async processLargeStreamEntry(entry, fullPath, options, result) {
        ...
        return new Promise((resolve, reject) => {
            entry.stream.on('data', async (chunk) => {
                // Check memory limit during processing
                if (this.memoryMonitor.isLimitExceeded()) {
                    entry.stream.destroy();
                    writeStream.destroy();
                    ...
                    reject(new zip_1.ZipExtractionError(`Memory limit exceeded during processing of large file ${entry.name}`, 'MEMORY_LIMIT_EXCEEDED'));
                    return;
                }
     ...
     */
    updateProcessingMetrics(bytesProcessed) {
        const now = Date.now();
     ...
     */
    getBuffer(size) {
        let pool = this.bufferPool;
        let maxPoolSize = this.MAX_POOL_SIZE;
     ...
     */
    async processEntries(zipFile, destinationPath, options, result) {
        const entries = [];
        const totalEntries = zipFile.entryCount;
        let processedEntries = 0;
        return new Promise((resolve, reject) => {
            ...
            zipFile.on('error', (error) => {
                zipFile.close();
                reject(new zip_1.ZipExtractionError(`Error during ZIP processing: ${error.message}`, 'EXTRACTION_FAILED', error));
            });
            // Start reading entries
            zipFile.readEntry();
        });
     ...
     */
    async extractEntry(zipFile, entry, destinationPath, zipEntry, options, result) {
        // Validate entry name to prevent path traversal
        if (!this.isValidEntryName(zipEntry.name)) {
            throw new zip_1.ZipExtractionError(`Invalid entry name: ${zipEntry.name}`, 'INVALID_ZIP_FILE');
        }
        ...
        return new Promise((resolve, reject) => {
            ...
            this.ensureDirectoryExists(path_1.default.dirname(fullPath))
                ...
                zipFile.openReadStream(entry, (err, readStream) => {
                    if (err) {
                        reject(new zip_1.ZipExtractionError(`Failed to open read stream for entry ${zipEntry.name}: ${err.message}`, 'EXTRACTION_FAILED', err));
                        return;
                    }
                    if (!readStream) {
                        reject(new zip_1.ZipExtractionError(`Failed to open read stream for entry ${zipEntry.name}: stream is null`, 'EXTRACTION_FAILED'));
                        return;
                    }
                    ...
                    readStream.on('data', (chunk) => {
                        ...
                        if (currentSize > maxSize) {
                            readStream.destroy();
                            writeStream.destroy();
                            reject(new zip_1.ZipExtractionError(`File ${zipEntry.name} exceeds size limit during extraction`, 'FILE_TOO_LARGE'));
                        }
                    });
                    ...
                    writeStream.on('error', (error) => {
                        reject(new zip_1.ZipExtractionError(`Failed to write file ${zipEntry.name}: ${error.message}`, 'EXTRACTION_FAILED', error));
                    });
                    readStream.on('error', (error) => {
                        reject(new zip_1.ZipExtractionError(`Failed to read entry ${zipEntry.name}: ${error.message}`, 'EXTRACTION_FAILED', error));
                    });
                });
            })
                .catch(error => {
                if (error instanceof zip_1.ZipExtractionError) {
                    reject(error);
                }
                else {
                    reject(new zip_1.ZipExtractionError(`Failed to extract entry ${zipEntry.name}: ${error instanceof Error ? error.message : 'Unknown error'}`, 'EXTRACTION_FAILED', error instanceof Error ? error : undefined));
                }
            });
        });
    }
    ...
    async ensureDirectoryExists(dirPath) {
        try {
            ...
            throw new zip_1.ZipExtractionError(`Failed to create directory ${dirPath}: ${error instanceof Error ? error.message : 'Unknown error'}`, 'EXTRACTION_FAILED', error instanceof Error ? error : undefined);
        }
    }
    /**
    ...
    setFilterConfig(config) {
        this.entryFilter = new EntryFilter_1.EntryFilter(config);
    }
}
exports.OptimizedZipProcessor = OptimizedZipProcessor;
OptimizedZipProcessor.EMPTY_STREAM = new stream_1.Readable({ read() { this.push(null); } });
//# sourceMappingURL=OptimizedZipProcessor.js.map