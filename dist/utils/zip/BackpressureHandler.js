...
class BackpressureHandler {
     ...
     */
    constructor(memoryLimit, defaultHighWaterMark) {
        this.memoryHistory = [];
        ...
        this.memoryMonitor = new MemoryMonitor_1.MemoryMonitor(memoryLimit !== undefined ? memoryLimit : 50 * 1024 * 1024); // 50MB default
     ...
     */
    applyBackpressure(stream, highWaterMark, options) {
        ...
        const backpressureTransform = new stream_1.Transform({
     ...
     */
    createControlledWritable(writeFunction, highWaterMark) {
     ...
     */
    monitorStreamFlow(readable, writable, options) {
     ...
     */
    applyAdaptiveBackpressure(stream, options) {
        const startTime = perf_hooks_1.performance.now();
        let chunkCount = 0;
        const watermark = options?.highWaterMark || this.calculateAdaptiveHighWaterMark();
        const adaptiveTransform = new stream_1.Transform({
            highWaterMark: watermark,
            transform: (chunk, encoding, callback) => {
                chunkCount++;
                // Update memory history for trend analysis
...
//# sourceMappingURL=BackpressureHandler.js.map