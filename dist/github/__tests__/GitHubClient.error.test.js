...
describe('GitHubClient Error Handling', () => {
    describe('Client Initialization Errors', () => {
        it('should handle invalid token format during initialization', () => {
            expect(() => new GitHubClient_1.GitHubClient('invalid-token')).toThrow('Invalid GitHub PAT format');
            expect(() => new GitHubClient_1.GitHubClient('')).toThrow('Invalid GitHub PAT format');
            expect(() => new GitHubClient_1.GitHubClient(null)).toThrow('Invalid GitHub PAT format');
        });
        it('should handle valid token format during initialization', () => {
            expect(() => new GitHubClient_1.GitHubClient('ghp_validtoken12345678901234567890123456')).not.toThrow();
        });
    });
    ...
    describe('Branch Service Errors', () => {
        ...
        beforeEach(() => {
            ...
            mockGetAuthHeaders = jest.fn().mockReturnValue({ 'Authorization': 'token test-token' });
            branchService = new BranchService_1.BranchService(mockHttpClient, mockGetAuthHeaders);
        });
    ...
    describe('HTTP Client Errors', () => {
        ...
        it('should handle timeout errors in HTTP client', async () => {
            const httpClient = new http_1.HttpClient({ timeout: 1 }); // 1ms timeout
            const repositoryService = new RepositoryService_1.RepositoryService(httpClient, mockGetAuthHeaders);
            // This will likely timeout
            await expect(repositoryService.list()).rejects.toThrow();
        });
        it('should handle invalid base URL errors', async () => {
            const httpClient = new http_1.HttpClient({ baseUrl: 'invalid-url' });
            const repositoryService = new RepositoryService_1.RepositoryService(httpClient, mockGetAuthHeaders);
            await expect(repositoryService.list()).rejects.toThrow();
        });
    });
});
//# sourceMappingURL=GitHubClient.error.test.js.map