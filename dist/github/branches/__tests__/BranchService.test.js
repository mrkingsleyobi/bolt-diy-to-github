...
describe('BranchService (London School TDD)', () => {
    ...
    describe('get', () => {
        it('should call httpClient.get with correct path and headers', async () => {
            // Arrange
            const mockBranch = { name: 'main', commit: { sha: 'abc123', url: 'url1' }, protected: true };
    ...
    describe('create', () => {
        it('should call httpClient.post with correct parameters', async () => {
            // Arrange
            const mockBranch = { name: 'feature', commit: { sha: 'xyz789', url: 'url3' }, protected: false };
            ...
            // Assert
            expect(mockHttpClient.post).toHaveBeenCalledWith('/repos/owner/repo/git/refs', branchData, { 'Authorization': 'token test-token' });
            expect(result).toEqual(mockBranch);
        });
