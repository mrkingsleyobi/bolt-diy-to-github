...
exports.demonstratePayloadEncryption = demonstratePayloadEncryption;
exports.crossOriginCommunicationExample = crossOriginCommunicationExample;
...
async function demonstratePayloadEncryption() {
    ...
    const encryptionService = new security_1.PayloadEncryptionService();
    ...
    try {
        ...
        const encryptedMessage = await encryptionService.encryptPayload(JSON.stringify(sensitiveData), secretKey);
        ...
        const decryptedPayload = await encryptionService.decryptPayload(receivedMessage, secretKey);
        ...
        const expiredMessage = await encryptionService.encryptPayload('This message will expire quickly', secretKey);
        // Wait for expiration
        ...
        try {
            ...
            console.log('✓ Expiration handled correctly:', error.message);
        }
...
async function crossOriginCommunicationExample() {
    ...
    const service = new security_1.PayloadEncryptionService();
    ...
    const encryptedOutgoing = await service.encryptPayload(JSON.stringify(outgoingMessage), secret);
...
//# sourceMappingURL=payload-encryption-example.js.map