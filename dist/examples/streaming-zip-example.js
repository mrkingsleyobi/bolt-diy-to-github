...
exports.basicStreamingExtraction = basicStreamingExtraction;
exports.memoryEfficientProcessing = memoryEfficientProcessing;
exports.entryFiltering = entryFiltering;
exports.completeIntegrationWorkflow = completeIntegrationWorkflow;
...
async function basicStreamingExtraction() {
    ...
    try {
        ...
        const extractor = new zip_1.StreamingZipExtractor(50 * 1024 * 1024);
...
async function memoryEfficientProcessing() {
    ...
    try {
        ...
        const processor = new zip_1.MemoryEfficientProcessor(100 * 1024 * 1024);
...
async function entryFiltering() {
    ...
    try {
        ...
        const filter = new zip_1.EntryFilter({
        ...
        const excludedEntries = sampleEntries.filter(entry => !filteredEntries.some(filtered => filtered.name === entry.name));
        console.log('\nExcluded entries:');
...
async function completeIntegrationWorkflow() {
    ...
    try {
        ...
        const extractor = new zip_1.StreamingZipExtractor(100 * 1024 * 1024); // 100MB
        const processor = new zip_1.MemoryEfficientProcessor(100 * 1024 * 1024);
        const handler = new zip_1.BackpressureHandler(100 * 1024 * 1024);
        const chunker = new zip_1.ChunkedProcessor(100 * 1024 * 1024);
        const filter = new zip_1.EntryFilter({
...
//# sourceMappingURL=streaming-zip-example.js.map