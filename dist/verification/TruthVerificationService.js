...
exports.TruthVerificationService = void 0;
/**
...
class TruthVerificationService {
     ...
     */
    verifyConfigurationResult(result, previousResult) {
     ...
     */
    calculateConsistencyScore(result, previousResult) {
        if (!result.config || !previousResult.config) {
            return 0.5; // Neutral score for missing configs
        }
        // Compare environment
        const environmentConsistent = result.config.environment === previousResult.config.environment ? 1 : 0;
        // Compare deployment target
        const deploymentTargetConsistent = result.config.deployment?.target === previousResult.config.deployment?.target ? 1 : 0;
        // Compare security settings
        const securityConsistent = result.config.security?.encryptionEnabled === previousResult.config.security?.encryptionEnabled ? 1 : 0;
    ...
    calculateWeightedScore(factors) {
        return (factors.validation * this.weights.validation +
     ...
     */
    meetsThreshold(score) {
        return score >= this.threshold;
    }
}
exports.TruthVerificationService = TruthVerificationService;
//# sourceMappingURL=TruthVerificationService.js.map